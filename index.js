...
/* `nestedFunction` is currently scoped inside `myFunction`, meaning that it has access to anything else scoped within
`myFunction`, but nothing outside of it. because the variable `internal` is declared within the `myFunction` scope, `nestedFunction`
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(animals){
    ...
    animals.forEach(animal => animalNames.push(`name: ${animal.animal_name}, scientific: ${animal.scientific_name}`));
  ...
  */
  ...
  function USApop(animals){
    return animals.reduce((acc, animal) => acc += animal.population, 0);
  }
